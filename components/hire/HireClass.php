...
class HireClass
{
    ...
    public static function checkHire()
    {
        ...
        foreach ($arr_eqs as $app_eq) {
            ...
            if (strtotime($app->rent_end) > date('Y-m-d H:i:s') && $app_eq->hire_state_id != 6) {
                $state = '2';
            }
            ...
            if ($state !== '') {
                ...
                try {
                    if (!$app_eq->save(false)) {
                        Yii::error('Ошибка при изменении состояния: ' . serialize($app_eq->getErrors()), __METHOD__);
                        return false;
                    }
                } catch (\Exception $e) {
                    Yii::error('Поймали Exception при изменении состояния: ' . serialize($e->getMessage()), __METHOD__);
                    return false;
                }
            }
        }
